...
do
  ...
  local function get_kernel_arg_info_address_qualifier(kernel, index)
    ...
    if status == C.CL_KERNEL_ARG_INFO_NOT_AVAILABLE then return "kernel arg info not available" end
    if status == C.CL_INVALID_VALUE or status == C.CL_INVALID_OPERATION or status == C.CL_KERNEL_ARG_INFO_NOT_AVAILABLE then return "invalid value" end
    if status ~= C.CL_SUCCESS then return error(errors[status]) end
...
function queue.enqueue_ndrange_kernel(queue, kernel, global_offset, global_size, local_size, events)
  ...
  if status ~= C.CL_SUCCESS then print("UNSUCCESSFUL"..status) end
  if status == C.CL_SUCCESS then print("SUCCESSFUL") end
  if status ~= C.CL_SUCCESS then return error(errors[status]) end
